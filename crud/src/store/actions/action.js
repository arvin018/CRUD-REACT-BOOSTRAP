...
//   const domain = 'http://localhost:3000'
  ...
  //fetch pokemon berarti user getAll
  ...
  export const postPokemon = (formPost) => {
    return async (dispatch, getState) => {
      try {
        ...
        // console.log("====================================");
        let response = await fetch(`${domain}/users/add`, {
          method: "POST",
          headers: {
        ...
        console.log(response,"ini add Users")
  ...
  export const editPokemon = (formEditPost,id) => {
    return async (dispatch, getState) => {
      try {
        ...
        console.log(response,"ini edit  users")
        dispatch(fetchPokemon());
      } catch (err) {
        dispatch(fetchRejectPokemon(err));
        console.log(err);
  ...
  export const deleteByIdPokemon = (id) => {
    return async (dispatch, getState) => {
      try {
          ...
          console.log(response,"ini delete  users")
        dispatch(fetchPokemon());
      } catch (err) {
        console.log(err);
      }
